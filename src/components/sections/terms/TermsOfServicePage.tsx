...
export function TermsOfServicePage() {
  ...
  const [activeSection, setActiveSection] = useState<string>(termsConfig.sections[0].id);
  ...
  useEffect(() => {
    ...
    termsConfig.sections.forEach(({ id }) => {
      ...
      <section className="relative min-h-[50vh] overflow-hidden sm:min-h-[55vh] md:min-h-[60vh]">
        ...
        <div className="relative z-10 mx-auto flex min-h-[50vh] flex-col items-center justify-center px-4 pt-24 pb-16 sm:min-h-[55vh] sm:pt-28 sm:pb-20 md:min-h-[60vh] md:pt-32 md:pb-24">
          ...
          <p className="mt-4 text-[14px] font-medium text-zinc-600 sm:text-[15px] md:text-[16px]">
            Effective Date: {termsConfig.effectiveDate}
          </p>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-start lg:gap-12">
            ...
            <aside
              ...
              style={{
                background: "linear-gradient(to bottom, #A363F420 0%, transparent 100%)",
              }}
            >
              <button
                type="button"
            ...
            <div className="min-w-0 flex-1 space-y-6">
              {termsConfig.sections.map((section) => (
                <article
                  key={section.id}
                  id={section.id}
                  className="scroll-mt-24 overflow-hidden rounded-2xl border border-zinc-200/80 bg-white p-[15px] md:p-[20px] shadow-sm"
                >
                  ...
                  <div className="mt-4 space-y-4">
                    {section.content.map((block, idx) =>
                      block.type === "paragraph" ? (
                        <p
                          key={idx}
                          className="whitespace-pre-line text-[14px] leading-relaxed text-zinc-900/60 sm:text-[15px] md:text-[16px]"
                        >
                          {block.text}
                        </p>
                      ) : block.type === "bullets" ? (
                        <ul
                          key={idx}
                            className="list-disc space-y-1 pl-5 text-[14px] leading-relaxed text-zinc-900/60 sm:text-[15px] md:text-[16px]"
                        >
